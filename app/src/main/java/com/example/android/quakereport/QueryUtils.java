...
public final class QueryUtils {
    ...
    public static ArrayList<EarthquakeDetail> fetchEarthquakeData(String requestUrl) {
        ...
        ArrayList<EarthquakeDetail> earthquakes;
    ...
    /**
     * Return a list of {@link EarthquakeDetail} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthquakeDetail> extractEarthquakes(String earthquakeJSON) {
        ...
        ArrayList<EarthquakeDetail> earthquakes = new ArrayList<>();
        ...
        try {
                ...
                for(int i = 0; i < features.length(); i++)
                {
                    ...
                    double magnitude = d.getDouble("mag");
                    String place = d.getString("place");
                    long time = d.getLong("time");

                    int in = place.indexOf(',');

                    String place_offset = null, main_place = null;

                    if(in == -1){
                        main_place = place;
                        place_offset = "Near the";
                    }else {
                        int y = place.indexOf('f');

                        main_place = place.substring(y+2);
                        place_offset = place.substring(0, y+1);
                    }
                    ...
                    String new_magnitude = decimalFormat.format(magnitude);

                    String urls = d.getString("url");

                    earthquakes.add(new EarthquakeDetail(new_magnitude, main_place, dateToDisplay, timeToDisplay, place_offset, urls));
                }
